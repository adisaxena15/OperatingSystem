...
void run_init(void) {
    ...
    // Open C:\shell (filesystem mount "c", file "shell")
    result = open_file(CMNTNAME, INITEXE, &initexe);
    if (result != 0) {
        kprintf(INITEXE ": %s; terminating\n", error_name(result));
        halt_failure();
    }
    // Simple argv: ["shell", NULL]
    argv[0] = INITEXE;
    argv[1] = NULL;
    
    // Run as a user process
    result = process_exec(initexe, 1, argv);
    // If process_exec fails, or if it returns unexpectedly
    if (result < 0) {
        kprintf("process_exec(" INITEXE ") failed: %s\n", error_name(result));
        halt_failure();
    }
    kprintf("process_exec(" INITEXE ") returned unexpectedly (%d)\n", result);
    halt_failure();
}
